...
The output is 62 New York counties, each its own <path>, with the eight
Northern New York counties flagged for accent fill. It is not rendered as-is:
tools/build_nny_map.py crops it into the compact map in the homepage hero.
...
Outputs: _includes/nc-counties.svg.html    (county paths)
         _data/map_meta.yml                 (viewBox + town x/y table)

Then run tools/build_nny_map.py to refresh the hero crop.
...
# Counties rendered in the accent fill: Northern New York, as defined in our
# positioning copy.
FOCUS = {
...
# _data/map_pins.yml. The extra Northern New York towns are pre-computed so a new
...
def main(src):
    ...
    # _includes/nny-map.html, which is hand-maintained, so that regenerating
...
PATHS_HEADER = '''<!-- Generated by tools/build_map.py - do not hand-edit.
     One <path> per New York county; the eight Northern New York counties
     carry .nc-county--focus. Source geometry for tools/build_nny_map.py,
     which crops it into the homepage hero map. -->
'''
