#!/usr/bin/env python3
"""
Regenerate the statewide county geometry and the town coordinate table.

The output is 62 New York counties, each its own <path>, with the North
Country counties flagged for accent fill. _includes/nys-map.html wraps it in
the shared statewide map used in the homepage hero and in "Why we're here".
Every colour is a CSS variable, so the map follows the theme rather than
baking one in. There is no map library.
...
Outputs: _includes/nys-counties.svg.html   (county paths)
         _data/map_meta.yml                 (viewBox + town x/y table)
...
# Counties rendered in the accent fill: the North Country, as the site
# defines it. Copy never counts them; the footer lists them by name.
FOCUS = {
...
def main(src):
    ...
    for name, rings in sorted(projected, key=lambda t: t[0]):
        ...
        if not d_parts:
            continue
        focus = ' nys-county--focus' if name in FOCUS else ''
        paths.append(
            '<path class="nys-county%s" data-county="%s" d="%s"></path>'
            % (focus, name, "".join(d_parts))
        )

    root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

    # Paths only. The <svg> wrapper, the Lowville star and the pins live in
    # _includes/nys-map.html, which is hand-maintained, so that regenerating
    # geometry never clobbers the pin list.
    out = os.path.join(root, "_includes", "nys-counties.svg.html")
    ...
    with open(meta, "w") as fh:
        ...
        fh.write("# geometry in _includes/nys-counties.svg.html. Pin x/y in\n")
...
PATHS_HEADER = '''<!-- Generated by tools/build_map.py - do not hand-edit.
     One <path> per New York county, 62 in all. The North Country counties
     carry .nys-county--focus. No <title> elements: the map is one role="img"
     with its own alt text, and native tooltips would give the static hero
     map a hover state. -->
'''
